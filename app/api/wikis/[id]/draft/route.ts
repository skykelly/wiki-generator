...
import { runDraftGeneration } from '@/lib/generate'
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!wiki.scaffold_result) return Response.json({ error: 'scaffold_result 없음' }, { status: 400 })
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        await runDraftGeneration(wikiId, (event) => send(event))
        send({ step: 'complete', message: '초안 생성 완료' })
      } catch (err) {
